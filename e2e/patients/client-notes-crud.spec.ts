 ...
 *
 * Create → Read → Update → Delete for general clinical notes.
 * Route: /app/client/$clientId/records/notes
 *
 * @tag @regression @patients @notes @crud
 */
...
const TS = Date.now();
const NOTE_TEXT    = `E2E clinical note ${TS.toString().slice(-6)}`;
const UPDATED_NOTE = `Updated ${NOTE_TEXT}`;

// ── Suite ─────────────────────────────────────────────────────────────────────

test.describe.serial('Client Clinical Notes — CRUD', () => {
  let clientId: string;
  ...
  test(
    'should display the Clinical Notes page @smoke',
    { annotation: [{ type: 'skipNetworkMonitoring' }] },
    async ({ page }) => {
      await goToNotes(page);

      const heading = page.getByText(/note|clinical note/i).first();
      await expect(heading).toBeVisible({ timeout: 10_000 });
    },
  );

  test(
    'should show notes list or empty state @smoke',
    { annotation: [{ type: 'skipNetworkMonitoring' }] },
    async ({ page }) => {
      await goToNotes(page);
  ...
  test(
    'should open Add Clinical Note form @smoke',
    { annotation: [{ type: 'skipNetworkMonitoring' }] },
    async ({ page }) => {
      await goToNotes(page);
  ...
  test(
    'should create a clinical note @smoke',
    { annotation: [{ type: 'skipNetworkMonitoring' }] },
    async ({ page }) => {
  ...
  test(
    'should edit the first clinical note',
    { annotation: [{ type: 'skipNetworkMonitoring' }] },
    async ({ page }) => {
      test.setTimeout(90_000);
      await goToNotes(page);
  ...
  test(
    'should delete the first clinical note',
    { annotation: [{ type: 'skipNetworkMonitoring' }] },
    async ({ page }) => {
      await goToNotes(page);
